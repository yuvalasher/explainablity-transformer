...
from nlp.BERT_explainability.modules.BERT.BertForSequenceClassification import Amit_Temp_BertForSequenceClassification, \
    BertForSequenceClassification as Test_BertForSequenceClassification
from nlp.BERT_explainability.modules.BERT.ExplanationGenerator import Generator
from nlp.BERT_rationale_benchmark.models.pipeline.bert_pipeline import initialize_models, \
    extract_docid_from_dataset_element
from nlp.BERT_rationale_benchmark.utils import annotations_from_jsonl
from nlp.BERT_rationale_benchmark.utils import (
    Annotation,
    Evidence,
    write_jsonl,
    load_datasets,
...
def initialize_models(params: dict, batch_first: bool, model_type: str, use_half_precision=False):
    assert batch_first
    ...
    evidence_classifier = bert_model_types[model_type].from_pretrained(bert_dir, num_labels=len(evidence_classes))
...
def main():
    ...
    evidence_classifier, word_interner, de_interner, evidence_classes, tokenizer = \
        initialize_models(model_params, batch_first=BATCH_FIRST, model_type=model_type)

    cache = os.path.join(args.output_dir, 'preprocessed_new.pkl')
    interned_documents = interning_documents(cache, documents, model_params, tokenizer)
    ...
    if os.path.exists(epoch_save_file):
        ...
        evidence_classifier.load_state_dict(torch.load(model_save_file, map_location=torch.device('cuda')))
    ...
    # test
    #
    test_classifier = Test_BertForSequenceClassification.from_pretrained(model_params['bert_dir'],
                                                                        num_labels=len(evidence_classes)).to(device)
    # test_classifier = Amit_Temp_BertForSequenceClassification.from_pretrained(model_params['bert_dir'],
    #                                                                           num_labels=len(evidence_classes)).to(
    #     device)
    ...
    if os.path.exists(epoch_save_file):
        ...
        for batch_start in range(0, len(test), test_batch_size):
            ...
            for s in batch_elements:
                ...
                output_path = os.path.join(args.output_dir, '{0}/{1}_GT_{2}_{3}.tex').format(
                    method_folder[method], j, classification, is_classification_correct)
                generate(text, cam_target, output_path)
                if method in ["transformer_attribution", "partial_lrp", "attn_gradcam", "lrp"]:
                    cam_false_class = \
                        method_expl[method](input_ids=input_ids, attention_mask=attention_masks, index=1 - target_idx)[
                            0]
                    cam_false_class = cam_false_class.clamp(min=0)
                    output_path = os.path.join(args.output_dir, '{0}/{1}_CF.tex').format(method_folder[method], j)
                    generate(text, cam_false_class, output_path)
...
def interning_documents(cache, documents, model_params, tokenizer):
