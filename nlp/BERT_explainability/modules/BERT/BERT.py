...
from nlp.BERT_explainability.modules.layers_ours import *
...
class Temp_BertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([Temp_BertLayer(config) for _ in range(config.num_hidden_layers)])

        self.gradient_checkpointing = False
        self.num_tokens = 512
        # TODO - change the device not to be hard-coded.
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if getattr(self.config, "gradient_checkpointing", False):
                ...
                layer_outputs = layer_module(
                    ...
                    layer_head_mask,
                    output_attentions,
                    temp=self.x_attention, layer_idx=i
                )
            hidden_states = layer_outputs[0]
            if output_attentions:
                all_attentions = all_attentions + (layer_outputs[1],)
...
class Temp_BertAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = Temp_BertSelfAttention(config, position_embedding_type=position_embedding_type)
        self.output = BertSelfOutput(config)
        self.pruned_heads = set()
        self.clone = Clone()
...
class Temp_BertSelfAttention(nn.Module):
    def __init__(self, config,position_embedding_type=None):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
...
class Temp_BertLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.attention = Temp_BertAttention(config)
        self.intermediate = BertIntermediate(config)
        self.output = BertOutput(config)
        self.clone = Clone()
...
class Temp_BertModel(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = BertEmbeddings(config)
        self.encoder = Temp_BertEncoder(config)
        self.pooler = BertPooler(config)
