...
from nlp.BERT_explainability.modules.layers_ours import Dropout, Linear, Tanh, LayerNorm, IndexSelect, Add, Clone
...
class BertTempEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.x_attention = nn.Parameter(
            torch.ones(config.num_hidden_layers, config.num_attention_heads, self.num_tokens, requires_grad=True,device='cuda'))
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module(
                    ...
                    past_key_value,
                    output_attentions,
                    temp=self.x_attention.to('cuda'), layer_idx=i
                )
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, BertTempEncoder):
...
class BertTempSoftmaxForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
