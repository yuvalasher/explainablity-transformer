...
def initialize_models(params: dict, batch_first: bool, model_type: str, use_half_precision=False):
    ...
    return  word_interner, de_interner, evidence_classes, tokenizer
...
def main():
    ...
    * decode first the evidence, then run classification for each split
    ...
    word_interner, de_interner, evidence_classes, tokenizer = \
        initialize_models(model_params, batch_first=BATCH_FIRST, model_type='infer')
    ...
    model_save_file = os.path.join(evidence_classifier_output_dir, 'classifier_hila.pt')
    ...
    device = 'cpu'
    print(" -------------- CPU -------------")
    temp_classifier = BertForSequenceClassification.from_pretrained(model_params['bert_dir'],
                                                                        num_labels=len(evidence_classes)).to(device)
    temp_classifier.load_state_dict(torch.load(model_save_file))
    ...
    test_classifier.bert.embeddings.load_state_dict(test_classifier.bert.embeddings.state_dict())
    test_classifier.bert.encoder.layer.load_state_dict(test_classifier.bert.encoder.layer.state_dict())
    test_classifier.bert.pooler.load_state_dict(test_classifier.bert.pooler.state_dict())
    test_classifier.dropout.load_state_dict(test_classifier.dropout.state_dict())
    test_classifier.classifier.load_state_dict(test_classifier.classifier.state_dict())
    ...
    if os.path.exists(epoch_save_file):
        ...
        j = 0
        for batch_start in tqdm(range(0, len(test), test_batch_size),position=0,leave=True):
            ...
            test_classifier.bert.encoder.x_attention = nn.Parameter(
                torch.ones(test_classifier.config.num_hidden_layers, test_classifier.config.num_attention_heads,
                           input_ids.shape[1], requires_grad=True,device=device))
