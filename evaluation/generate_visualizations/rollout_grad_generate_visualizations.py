...
def compute_saliency_and_save(results_path: Path, feature_extractor: ViTFeatureExtractor, vit_model: nn.Module):
    ...
    with h5py.File(results_path, 'a') as f:
        data_cam_rollout_max_grad = f.create_dataset('vis_rollout_max_grad',
                                                     VIS_SHAPE,
                                                     maxshape=VIS_MAX_SHAPE,
                                                     dtype=np.float32,
                                                     compression="gzip")
        data_cam_rollout_mean_relu_grad = f.create_dataset('vis_rollout_mean_relu_grad',
                                                           VIS_SHAPE,
                                                           maxshape=VIS_MAX_SHAPE,
                                                           dtype=np.float32,
                                                           compression="gzip")
        ...
        for batch_idx, (data, target) in enumerate(tqdm(sample_loader)):
            ...
            resize_array_src_to_dst_shape(src_array=data_cam_rollout_max_grad, dst_array_shape=data.shape,
                                          is_first=first)
            resize_array_src_to_dst_shape(src_array=data_cam_rollout_mean_relu_grad, dst_array_shape=data.shape,
                                          is_first=first)
            ...
            insert_result_to_array(res_by_type['rollout_mean_relu_grad'], array=data_cam_rollout_max_grad,
                                   data_shape=data.shape)
            insert_result_to_array(res_by_type['rollout_max_grad'], array=data_cam_rollout_mean_relu_grad,
                                   data_shape=data.shape)
