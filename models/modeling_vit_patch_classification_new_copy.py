...
class ViTForMaskGeneration(ViTPreTrainedModel):
    ...
    def extract_patch_logits(self, tokens_output):
        logits = self.patch_classifier(self.activation(self.patch_pooler(tokens_output.reshape(-1, tokens_output.shape[-1]))))
    ...
    def generate_mask(self, logits, batch_size):
        ...
        interpolated_mask = torch.nn.functional.interpolate(
            sampled_mask, scale_factor=vit_config["patch_size"], mode='nearest'
        )
    ...
    def prepare_inpainting_inputs(self, pixel_values, binary_mask):
        ...
        images_pil = [to_pil_image(pixel_values[i].cpu()).resize((512, 512), Image.BICUBIC) for i in range(pixel_values.shape[0])]
        masks_pil = [to_pil_image(binary_mask[i][0].cpu()).resize((512, 512), Image.NEAREST) for i in range(binary_mask.shape[0])]
        return images_pil, masks_pil

    def get_class_prompt(self, top2_classes, i):
        original_idx, second_idx = (top2_classes.indices[i][0].item(), top2_classes.indices[i][1].item()) if top2_classes is not None else (0, 0)
    ...
    def perform_inpainting_batch(self, pixel_values, logits, batch_size, top2_classes):
        ...
        inpainted_images = [self.run_inpainting(images_pil[i], masks_pil[i], self.get_class_prompt(top2_classes, i)[0]) for i in range(batch_size)]
    ...
    def forward(
        ...
        if self.use_inpaint_model:
            sampled_mask, interpolated_mask, inpainted_image = self.perform_inpainting_batch(pixel_values, logits, batch_size, top2_classes)
        else:
            sampled_mask, interpolated_mask, inpainted_image = self.generate_mask_without_inpainting(logits, batch_size, tokens_count)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        interpolated_mask, sampled_mask, inpainted_image = model(pixel_values=image_tensor, use_inpainting_mask=True,
                                                                 top2_classes=top2)
