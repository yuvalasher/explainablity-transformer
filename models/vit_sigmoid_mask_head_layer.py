...
class ViTSelfAttention(nn.Module):
    ...
    def forward(
        ...
        attention_probs = (
            sampled_binary_patches[layer_idx].unsqueeze(0).unsqueeze(2) * attention_probs
        )  # element-wise mul.
...
class ViTLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = (
            self.attention(  # run VitAttention forward, inside run the self-attention forward
...
class ViTEncoder(nn.Module):
    ...
    def calculate_sampled_distribution_gumble_softmax(self, x_attention) -> torch.Tensor:
        ...
        ).reshape(n_layers, n_heads, n_tokens, 2)
        # print(log_prob_x_attention.shape)
        log_probs = [
            gumbel_softmax(p_dist, temperature=1)
            for log_prob in log_prob_x_attention
            for p_dist in log_prob
        ]
        sampled_binary_patches = (torch.stack(log_probs) @ torch.tensor([0.0, 1.0]).T).reshape(
            n_layers, n_heads, n_tokens
        )
        sampled_binary_patches[:, :, 0] = torch.tensor(1.0)
...
class ViTSigmoidPerLayerHeadForImageClassification(ViTPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
