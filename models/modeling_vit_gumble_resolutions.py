...
class ViTSelfAttention(nn.Module):
    ...
    def forward(
        ...
        )  # [batch_size, n_heads, num_patches + 1,\
        # attention_head_size]
...
class ViTEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.x_attention = nn.Parameter(
            torch.randn(36, requires_grad=True)
        )  # [n_patches + 1 for [CLS]]
    ...
    def calculate_sampled_distribution_gumble_softmax(self, x_attention) -> torch.Tensor:
        ...
        log_prob_x_attention = torch.stack(
            (torch.log(1 - sigmoid_x_attention), torch.log(sigmoid_x_attention))
        ).T
        log_probs = [
            gumbel_softmax(log_prob, temperature=vit_config["temperature"])
            for log_prob in log_prob_x_attention
        ]
        sampled_binary_patches = torch.stack(log_probs) @ torch.tensor([0.0, 1.0]).T
    ...
    def get_sampled_binary_patches_original_resolution(self, sampled_binary_patches):
        ...
        for block_idx in range(int(num_patches_in_row / resolution_factor)):
            for res_factor in range(resolution_factor):
                sampled_binary_patches_original_resolution.extend(
                    sampled_binary_patches_interpolated[0, 0, 0][
                        block_idx * num_patches_in_row : (block_idx + 1) * num_patches_in_row
                    ]
                )
        sampled_binary_patches_original_resolution = torch.cat(
            (
                torch.tensor(1.0).unsqueeze(0),
                torch.tensor(sampled_binary_patches_original_resolution),
            )
        )
        return sampled_binary_patches_original_resolution
    ...
    def forward(
        ...
        if tokens_mask is not None:
            ...
            self.sampled_binary_patches = self.get_sampled_binary_patches_original_resolution(
...
class ViTGumbleResolutionsForImageClassification(ViTPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
