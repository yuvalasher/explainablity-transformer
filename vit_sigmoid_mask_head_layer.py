...
class ViTLayer(nn.Module):
    ...
    def forward(self, hidden_states, head_mask=None, output_attentions=False, sampled_binary_patches=None,
                layer_idx=None):
        self_attention_outputs = self.attention(  # run VitAttention forward, inside run the self-attention forward
...
class ViTEncoder(nn.Module):
    ...
    def calculate_sampled_distribution_gumble_softmax(self, x_attention) -> torch.Tensor:
        ...
        log_prob_x_attention = torch.stack(
            (torch.log(1 - sigmoid_x_attention), torch.log(sigmoid_x_attention))).reshape(n_layers, n_heads, n_tokens,
                                                                                          2)
        # print(log_prob_x_attention.shape)
        log_probs = [gumbel_softmax(p_dist, temperature=1) for log_prob in log_prob_x_attention for p_dist in log_prob]
        sampled_binary_patches = (torch.stack(log_probs) @ torch.tensor([0., 1.]).T).reshape(n_layers, n_heads,
                                                                                             n_tokens)
