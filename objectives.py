...
def objective_temp_softmax(
    output: Tensor, target: Tensor, temp: Tensor, contrastive_class_idx: Tensor = None
) -> Tensor:
    target_idx = (
        contrastive_class_idx if contrastive_class_idx is not None else torch.argmax(target)
    )
    ...
    log(
        ...
        output=output,
        target=target,
        contrastive_class_idx=target_idx.item(),
    )
    return loss
