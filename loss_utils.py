...
def prediction_loss_plus_bce_turn_off_patches_loss(
    output: Tensor, target: Tensor, x_attention: Tensor, iteration_idx: int
) -> Tensor:
    """
    ...
    prediction_loss = ce_loss(
        output, torch.argmax(target).unsqueeze(0)
    )  # maximize the pred to original model
    bce_turn_off_patches_loss = bce_with_logits_loss(
        x_attention, target_off_patches
    )  # turn off patches
    print(
        f"prediction_loss: {prediction_loss}, bce_turn_off_patches_loss: {bce_turn_off_patches_loss}"
    )
...
def log(
    ...
    other_loss=None,
    contrastive_class_idx: int = None,
) -> None:
    target_class_idx = (
        contrastive_class_idx
        if contrastive_class_idx is not None
        else torch.argmax(F.softmax(target, dim=-1)).item()
    )
    if vit_config["log"]:
        wandb.log(
            {
...
def print_objective_every(
    a: Tensor, b: Tensor, iteration_idx: int, output: Tensor, target: Tensor
) -> None:
