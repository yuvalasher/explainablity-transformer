...
from evaluation.perturbation_tests.head_perturbation_tests import eval, get_precision_at_k_by_k_heads, \
    get_gt_heads_order
...
def optimize_params(vit_model: ViTForImageClassification, vit_ours_model, feature_extractor, criterion: Callable):
    df = pd.DataFrame(columns=['grad_min_v', 'grad_max_v', 'grad_mean_v', 'grad_median_v', 'relu_grad_max_v',
                               'relu_grad_mean_v', 'relu_grad_median_v'])
    ...
    for idx, image_dict in enumerate(vit_config['images']):
        ...
        context_layer_grad = vit_unfreezed.vit.encoder.layer[-1].attention.attention.context_layer.grad[0, 0].reshape(
            n_heads, -1)
        ...
        if not os.path.isfile(Path(objects_path, 'gt_heads_order.pkl')):
            change_predicted_class_by_head = eval(experiment_dir=image_plot_folder_path, model=vit_unfreezed,
                                                  feature_extractor=feature_extractor,
                                                  heads_masks=cls_attentions_probs,
                                                  image=resized_image)
            gt_order_heads = get_gt_heads_order(change_predicted_class_by_head=change_predicted_class_by_head)
        else:
        ...
        rollout_mean_relu_grad_6_heads = rollout(attentions=attn, head_fusion='mean', gradients=att_relu_grads)
        rollout_max_grad_12_heads = rollout(attentions=attention_probs, head_fusion='max', gradients=gradients)
        rollout_mean_relu_grad_12_heads = rollout(attentions=attention_probs, head_fusion='mean',
                                                  gradients=relu_gradients)
        rollout_mean_grad_12_heads = rollout(attentions=attention_probs, head_fusion='mean', gradients=gradients)
        rollout_mean_grad_6_heads = rollout(attentions=attn, head_fusion='mean', gradients=att_grads)
        ...
        """
        change_predicted_mean_grad_rollout_12_heads_iter = \
        ...
        change_predicted_mean_grad_rollout_6_heads_iter = \
            eval(experiment_dir=image_plot_folder_path, model=vit_unfreezed,
                 feature_extractor=feature_extractor,
                 heads_masks=rollout_mean_grad_6_heads.reshape(-1),
                 image=resized_image)[0]
        change_predicted_max_grad_rollout_6_heads_iter = \
            eval(experiment_dir=image_plot_folder_path, model=vit_unfreezed,
                 feature_extractor=feature_extractor,
                 heads_masks=rollout_max_grad_6_heads.reshape(-1),
                 image=resized_image)[0]

        change_predicted_max_grad_rollout_12_heads_iter = \
            eval(experiment_dir=image_plot_folder_path, model=vit_unfreezed,
                 feature_extractor=feature_extractor,
                 heads_masks=rollout_max_grad_12_heads.reshape(-1),
                 image=resized_image)[0]
        change_predicted_mean_relu_grad_rollout_6_heads_iter = \
            eval(experiment_dir=image_plot_folder_path, model=vit_unfreezed,
                 feature_extractor=feature_extractor,
                 heads_masks=rollout_mean_relu_grad_6_heads.reshape(-1),
                 image=resized_image)[0]

        change_predicted_mean_relu_grad_rollout_12_heads_iter = \
            eval(experiment_dir=image_plot_folder_path, model=vit_unfreezed,
                 feature_extractor=feature_extractor,
        ...
        plot_heads_scores(image_name=image_name, scores=torch.topk(relu_grad_mean_v, n_heads, largest=True)[0].tolist(),
                          rank_method='relu_grad_mean_v', path=image_plot_folder_path)
        plot_heads_scores(image_name=image_name, scores=torch.topk(grad_max_v, n_heads, largest=True)[0].tolist(),
                          rank_method='grad_max_v', path=image_plot_folder_path)
        save_obj_to_disk(path=Path(objects_path, 'gt_heads_order'), obj=gt_order_heads)
        save_obj_to_disk(path=Path(objects_path, 'relu_grad_mean_v_order'), obj=relu_grad_mean_v_order)
        ...
        predicted_change_df.to_csv(Path(PLOTS_PATH, experiment_name, 'predicted_change_df.csv'), index=False)
