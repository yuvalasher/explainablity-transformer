...
def optimize_params(
    ...
    for idx, image_dict in enumerate(vit_config["images"]):
        ...
        priors = torch.cat((torch.ones(1), rollout_max_grad)).expand(
            vit_model.config.num_hidden_layers,
        ...
        for iteration_idx in tqdm(range(vit_config["num_steps"])):
            ...
            (
                ...
                correct_class_prob,
                prediction_loss,
            ) = get_iteration_target_class_stats(output=output, target_class_idx=target_class_idx)
            loss = criterion(
            ...
            if vit_config["plot_visualizations"]:
                visualize_attention_scores_with_rollout(
