        ...
        with wandb.init(project="vit-temp-softmax", entity="yuvalasher", config=wandb_config) as run:
            ...
            for iteration_idx in tqdm(range(vit_config['num_steps'])):
                ...
                visualize_attentions_and_temps(cls_attentions_probs=cls_attentions_probs, iteration_idx=iteration_idx,
                                               ...
                                               temp_tokens_min_folder=temp_tokens_min_folder, temp_tokens_folder=temp_tokens_folder)
...
if __name__ == '__main__':
    ...
    experiment_name = f"vis_1d_mul_{vit_config['objective']}_lr{str(vit_config['lr']).replace('.', '_')}+l1_{loss_config['l1_loss_multiplier']}+kl_loss_{loss_config['kl_loss_multiplier']}+entropy_loss_{loss_config['entropy_loss_multiplier']}+pred_loss_{loss_config['pred_loss_multiplier']}"
