...
# CONTRASTIVE_CLASS_IDX = torch.tensor(340)
...
def objective_temp_bias_softmax(output: Tensor, target: Tensor, temp: Tensor, contrastive_class_idx: Tensor=None) -> Tensor:
    ...
    for idx, image_dict in enumerate(vit_config['images']):
        ...
        if contrastive_class_idx is not None:
            ...
            image_plot_folder_path = get_and_create_image_plot_folder_path(images_folder_path=IMAGES_FOLDER_PATH,
                                                                           ...
                                                                           image_name=image_name, is_contrastive_run=True)
