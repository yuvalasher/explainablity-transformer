...
BASE_AUC_OBJECTS_PATH = Path(RESULTS_PICKLES_FOLDER_PATH,
                             target_or_predicted_model)  # /raid/yuvalas/experiments/<target/predicted>
...
if __name__ == '__main__':
    ...
    CAT_DOG_FOLDER = "/home/yuvalas/explainability/data/2_classes"
    IMAGES_PATH = CAT_DOG_FOLDER
    ic(exp_name)
    ...
    targets = [101, 304, 207, 285, 340, 386, 243, 282, 161, 88]

    # IMGES_PATH = IMAGENET_VAL_IMAGES_FOLDER_PATH
    # ic(exp_name)
    # print(f"Total Images in path: {len(os.listdir(IMAGES_PATH))}")
    # ic(vit_config['lr'], loss_multipliers["mask_loss_mul"], loss_multipliers["prediction_loss_mul"])
    # start_time = dt.now()
