...
def optimize_params(vit_model: ViTForImageClassification, criterion: Callable):
    for idx, image_dict in enumerate(vit_config['images']):
        ...
        image_plot_folder_path = get_and_create_image_plot_folder_path(images_folder_path=IMAGES_FOLDER_PATH,
                                                                       ...
                                                                       image_name=image_name, save_image=True)
        ...
        for iteration_idx in tqdm(range(vit_config['num_steps'])):
            ...
            if iteration_idx == 0:
                ...
                save_obj_to_disk(Path(iter_0_folder, 'tokens_mask.pkl'), cls_attentions_probs_iter_0)
                ...
                for head_idx in range(12):
                    visu(original_image=original_transformed_image,
                         transformer_attribution=cls_attentions_probs[head_idx],
                         file_name=Path(iter_advanced_folder, f'cls_head_{head_idx}_iter_{iteration_idx}'))
                save_obj_to_disk(Path(iter_advanced_folder, 'tokens_mask.pkl'), cls_attentions_probs_iter_advanced)
            elif iteration_idx == 199:
                print(1)
            if vit_config['plot_visualizations'] and iteration_idx >= 160 and iteration_idx % 10 == 0:
                for head_idx in range(12):
                    entropy_iter_0 = round(entropy(cls_attentions_probs_iter_0[head_idx]).item(), 3)
                    entropy_iter_advanced = round(entropy(cls_attentions_probs_iter_advanced[head_idx]).item(), 3)
                ...
                for head_idx in range(12):
                    entropy_iter_0 = round(entropy(F.softmax(temp_iter_0[head_idx], dim=-1)).item(), 3)
                    entropy_iter_advanced = round(entropy(F.softmax(temp_iter_advanced[head_idx], dim=-1)).item(), 3)
                    diff = round(entropy_iter_0 - entropy_iter_advanced, 3)
                    print(
