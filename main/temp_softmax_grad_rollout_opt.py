...
def temp_softmax_grad_rollout_optimization(
    vit_ours_model,
    vit_model,
    ...
    (
        ...
        correct_class_probs,
        tokens_mask_rollout_max_grad,
        tokens_mask_rollout_mean_relu_grad,
        temps,
    ) = ([], [], [], [], [], [], [])
    ...
    for iteration_idx in range(num_steps):
        ...
        cls_attentions_probs = get_attention_probs_by_layer_of_the_CLS(
            model=vit_ours_model, layer=-1
        )
        token_mask_rollout_mean_relu_grad = (
            cls_attentions_probs.clone() * d_masks["rollout_mean_relu_grad"]
        )
    ...
    cls_attn_probs_by_stop_points = {
        "min_pred_loss_rollout_grad_max": tokens_mask_rollout_max_grad[min_pred_loss_iter],
        ...
        "max_logits_rollout_mean_relu_grad": tokens_mask_rollout_mean_relu_grad[max_logits_iter],
    }
    for iter_idx in [90, 100, 110, 120, 130, 140, 150, 160, 165, 170, 175, 180, 185, 190]:
        cls_attn_probs_by_stop_points[
            f"iter_{iter_idx}_rollout_grad_max"
        ] = tokens_mask_rollout_max_grad[iter_idx]
        cls_attn_probs_by_stop_points[
            f"iter_{iter_idx}_rollout_mean_relu_grad"
        ] = tokens_mask_rollout_mean_relu_grad[iter_idx]
